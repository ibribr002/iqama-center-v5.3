    ...
    return (
        <Layout user={user}>
            ...
            <div className="messages-section">
                <h2><i className="fas fa-comments fa-fw"></i> مساحة المشاركة والتواصل</h2>
                
                <form onSubmit={handleSendMessage} className="message-form">
                    <textarea
                        value={newMessage}
                        onChange={(e) => setNewMessage(e.target.value)}
                        placeholder="شارك أفكارك أو اطرح سؤالاً..."
    ...
    try {
        ...
        try {
            examsRes = await pool.query(
                'SELECT * FROM exams WHERE course_id = $1 ORDER BY start_time', 
                [id]
            );
            ...
            try {
                examsRes = await pool.query(
                    'SELECT * FROM course_exams WHERE course_id = $1 ORDER BY created_at', 
                    [id]
                );
                console.log(`Found ${examsRes.rows.length} course_exams for course ${id}:`, examsRes.rows);
            } catch (altError) {
