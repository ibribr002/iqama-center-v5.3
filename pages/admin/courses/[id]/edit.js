                            ...
                            <form onSubmit={handleCreateExam}>
                                ...
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setExamForm({...examForm, passingGrade: parseInt(e.target.value)})}
                                        className="form-control"
                                        min="0"
                                        max="100"
                                    />
                            ...
                            exams.map(exam => (
                                <div key={exam.id} className="exam-card">
                                    ...
                                    <div className="exam-meta">
                                        ...
                                        <p><strong>وقت البداية:</strong> {new Date(exam.start_time).toLocaleString('ar-EG')}</p>
                                        <p><strong>وقت النهاية:</strong> {new Date(exam.end_time).toLocaleString('ar-EG')}</p>
                                        <p><strong>درجة النجاح:</strong> {exam.passing_grade}%</p>
                                    </div>
...
export async function getServerSideProps(context) {
    ...
    try {
        ...
        try {
            examsResult = await pool.query('SELECT * FROM exams WHERE course_id = $1 ORDER BY start_time', [id]);
        } catch (error) {
