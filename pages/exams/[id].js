...
const ExamPage = ({ user, exam, course }) => {
        ...
        if (exam && exam.end_time) {
            ...
            if (now < endTime) {
                ...
                const timer = setInterval(() => {
                    const newRemaining = endTime.getTime() - new Date().getTime();
                    if (newRemaining <= 0) {
                        setTimeRemaining(0);
                        clearInterval(timer);
    ...
    if (now < startTime) {
        return (
            <Layout user={user}>
                <div style={{ textAlign: 'center', padding: '50px' }}>
                    <h2>الامتحان لم يبدأ بعد</h2>
                    <p>سيبدأ الامتحان في: {startTime.toLocaleString('ar-EG')}</p>
                    <button onClick={() => router.back()}>العودة</button>
                </div>
            </Layout>
        );
        ...
        return (
            <Layout user={user}>
                <div style={{ textAlign: 'center', padding: '50px' }}>
                    ...
                    <p>انتهى الامتحان في: {endTime.toLocaleString('ar-EG')}</p>
                    <button onClick={() => router.back()}>العودة</button>
                </div>
            </Layout>
        );
